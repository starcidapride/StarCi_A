...
public class Peura : ICharacterCard
{
    ...
    public Texture2D PassiveImage { get; } = Resources.Load<Texture2D>(PEURA_IMAGE + "Passive");

    public string PassiveDescription { get; } = "TC";

    public string QName { get; } = "Eagle Eye";

    public Texture2D QImage { get; } = Resources.Load<Texture2D>(PEURA_IMAGE + "Q");

    public string QDescription { get; } = "ASTT";

    public string EName { get; } = "Penetrating Shot";

    public Texture2D EImage { get; } = Resources.Load<Texture2D>(PEURA_IMAGE + "E");

    public string EDescription { get; } = "CMP";

    public string RName { get; } = "Arrow Of Chaos";

    public Texture2D RImage { get; } = Resources.Load<Texture2D>(PEURA_IMAGE + "R");
